...
for i in range(len(content)):
    # Fix Latex equations
    if "\\begin{align*}" in content[i] or "$$" in content[i]:
        in_eq = True
    if ("\\end{align*}" in content[i] or "$$" in content[i]) and in_eq:
        in_eq = False
    if in_eq:
        print(markdown['text'][content_df['index'][i]])
        markdown['text'][content_df['index'][i]] = re.sub(
            r"([^\\])\\\\$", r"\1 \\\\\\", content[i]
            )
        markdown['text'][content_df['index'][i]] = re.sub(
            r"\\sum_", r"\\sum\\_", markdown['text'][content_df['index'][i]]
            )
        print(markdown['text'][content_df['index'][i]])
    # Fix images that come after hidden cells
    elif "{{< /details >}}" in content[i] and "![png]" in content[i-1]:
